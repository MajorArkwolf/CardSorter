...
void readAndReportData(byte address, int theRegister, byte numBytes,
  ...
  // available do not always require the register read so upon interrupt you
  // call Wire.requestFrom()
...
void setPinModeCallback(byte pin, int mode) {
  ...
  if (Firmata.getPinMode(pin) == PIN_MODE_I2C && isI2CEnabled &&
      mode != PIN_MODE_I2C) {
  ...
  if (IS_PIN_ANALOG(pin)) {
    reportAnalogCallback(PIN_TO_ANALOG(pin), mode == PIN_MODE_ANALOG
                                                 ? 1
...
/*
 * Sets the value of an individual pin. Useful if you want to set a pin value
 * but are not tracking the digital port state. Can only be used on pins
 * configured as OUTPUT. Cannot be used to enable pull-ups on Digital INPUT
 * pins.
 */
...
#else
            // only write to the INPUT pin to enable pullups if Arduino v1.0.0
            // or earlier
            pinWriteMask |= mask;
#endif
          }
...
void setup() {
  ...
  // Mega, Call begin(baud) on the alternate serial port and pass it to Firmata
  // to begin like this: Serial1.begin(57600); Firmata.begin(Serial1); However
  // do not do this if you are using SERIAL_MESSAGE
